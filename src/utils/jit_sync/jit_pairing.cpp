...
//bool jit_pairing_event_cb( EventBits_t event, void *arg );
...
void jit_pairing_tile_setup( void ) {
    ...
    lv_style_set_text_font( &jit_pairing_style, LV_STATE_DEFAULT, &Ubuntu_32px);
    ...
    lv_img_set_src( jit_pairing_img, &bluetooth_64px );
    lv_obj_align( jit_pairing_img, jit_pairing_tile, LV_ALIGN_CENTER, 0, 0 );
    ...
    lv_label_set_text( jit_pairing_info_label, "");
...
bool jit_pairing_event_cb( EventBits_t event, void *arg ) {
    switch( event ) {
        case BLECTL_PIN_AUTH:           statusbar_hide( true );
                                        ...
                                        lv_label_set_text( jit_pairing_info_label, (const char*)arg );
                                        lv_obj_align( jit_pairing_info_label, jit_pairing_img, LV_ALIGN_OUT_BOTTOM_MID, 0, 5 );
...
bool jit_pairing_event_cb() {
    ...
    switch( event ) {
        case BLECTL_PIN_AUTH:           statusbar_hide( true );
                                        powermgm_set_event( POWERMGM_WAKEUP_REQUEST );
                                        mainbar_jump_to_tilenumber( jit_pairing_tile_num, LV_ANIM_OFF );
                                        lv_label_set_text( jit_pairing_info_label, "teste" );
                                        lv_obj_align( jit_pairing_info_label, jit_pairing_img, LV_ALIGN_OUT_BOTTOM_MID, 0, 5 );
                                        lv_obj_invalidate( lv_scr_act() );
                                        motor_vibe(20);
                                        break;
