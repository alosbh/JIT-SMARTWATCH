#include "config.h"
#include <TTGO.h>
#include "quickglui/quickglui.h"

#include "dashboard.h"
#include "gui/mainbar/mainbar.h"
#include "gui/widget_styles.h"
#include "hardware/bma.h"
#include "hardware/blestepctl.h"
#include "hardware/motor.h"

// App icon must have an size of 64x64 pixel with an alpha channel
// Use https://lvgl.io/tools/imageconverter to convert your images and set "true color with alpha"
LV_IMG_DECLARE(dashboard);
LV_IMG_DECLARE(trash_32px);
LV_FONT_DECLARE(Ubuntu_16px);
LV_FONT_DECLARE(Ubuntu_32px);

#define YES "Yes"
#define NO  "No"

static SynchronizedApplication dashboardApp;
static JsonConfig config("dashboard.json");

// Options
static String size, length, goal_step, goal_dist;
// Widgets
static Label lblStepcounter, lblStepachievement;
static Label lblArea;
static Label lblLine;
static Label lblUPH;

static Label lblDistance, lblDistance2, lblDistachievement,lblDistachievement2;
static Arc  arcDistance, arcDistance2;

static Style big, small;

/* Default message box callback */
static lv_event_cb_t default_msgbox_cb;

static void build_main_page();
static void refresh_main_page();
static void build_settings();


static void dashboard_activate_cb();
static void dashboard_reset_cb(lv_obj_t * obj, lv_event_t event);

/*
 * setup routine for application
 */
void dashboard_app_setup() {
    // Create and register new application
    //   params: name, icon, auto add "refresh" button (this app will use synchronize function of the SynchronizedApplication class).
    //   Also, you can configure count of the required pages in the next two params (to have more app screens).
    dashboardApp.init("Dashboard", &dashboard, 1, 1);

    mainbar_add_tile_activate_cb( dashboardApp.mainTileId(), dashboard_activate_cb );

    // Build and configure application
    build_main_page();
    build_settings();

    // Executed when user click "refresh" button
    dashboardApp.synchronizeActionHandler([](SyncRequestSource source) {
       // if ( blectl_get_event( BLECTL_ON ) )
       // {
      //      blestepctl_update(true);
      //  }
        // Return feedback to user as nothing else changed
        motor_vibe(20);
    });


/*
    // Add a trash button to reset counter
    dashboardApp.mainPage().addAppButton(trash_32px, [](Widget btn) {
        static const char * btns[] ={YES, NO, ""};

        lv_obj_t * mbox1 = lv_msgbox_create(lv_scr_act(), NULL);
        lv_msgbox_set_text(mbox1, "Reset step counter?");
        lv_msgbox_add_btns(mbox1, btns);
        lv_obj_set_width(mbox1, 200);
        // Save default callback
        default_msgbox_cb = lv_obj_get_event_cb(mbox1);
        lv_obj_set_event_cb(mbox1, dashboard_reset_cb);
        lv_obj_align(mbox1, NULL, LV_ALIGN_CENTER, 0, 0);
    });*/
    
    refresh_main_page();
}

void build_main_page()
{
    
    big = Style::Create(ws_get_mainbar_style(), true);
    big.textFont(&Ubuntu_32px)
      .textOpacity(LV_OPA_80);
    
    small = Style::Create(ws_get_mainbar_style(), true);
    small.textFont(&Ubuntu_16px)
      .textOpacity(LV_OPA_80);


    AppPage& screen = dashboardApp.mainPage(); // This is parent for all main screen widgets



    arcDistance = Arc(&screen, 0, 360);
    arcDistance.start(0).end(0).rotation(90)
        .style(ws_get_arc_style(), LV_ARC_PART_INDIC, false )
        .style(ws_get_arc_bg_style(), LV_ARC_PART_BG, false )
        .size(120, 120)
        .alignInParentBottomLeft(0, -42);        

    lblDistance = Label(&screen);
    lblDistance.text("0")
        .style(small, true)
        .align(arcDistance, LV_ALIGN_OUT_TOP_MID);
    
    lblDistachievement = Label(&screen);
    lblDistachievement.text("0")
        .style(small, true)
        .alignOrig0(arcDistance, LV_ALIGN_CENTER);


    arcDistance2 = Arc(&screen, 0, 360);
    arcDistance2.start(0).end(0).rotation(90)
        .style(ws_get_arc_style(), LV_ARC_PART_INDIC, false )
        .style(ws_get_arc_bg_style(), LV_ARC_PART_BG, false )
        .size(120,120)
        .alignInParentBottomRight(0, -42);        

    lblDistance2 = Label(&screen);
    lblDistance2.text("0")
        .style(small, true)
        .align(arcDistance2, LV_ALIGN_OUT_TOP_MID);
    
    lblDistachievement2 = Label(&screen);
    lblDistachievement2.text("0")
        .style(small, true)
        .alignOrig0(arcDistance2, LV_ALIGN_CENTER);

    lblArea= Label(&screen);
    lblArea.text("Area")
    .style(small, true)
    .alignOrig0(screen, LV_ALIGN_IN_TOP_LEFT, 20,10);

    lblLine= Label(&screen);
    lblLine.text("Line")
    .style(small, true)
    .alignOrig0(screen, LV_ALIGN_IN_TOP_LEFT, 20,25);

    lblUPH= Label(&screen);
    lblUPH.text("UPH")
    .style(small, true)
    .alignOrig0(screen, LV_ALIGN_IN_TOP_LEFT, 20,45);
}


uint8_t get_fpy()
{
    uint8_t aux=1;

return 0;
}



void refresh_main_page()
{
    char buff[36];
    uint32_t gStep = goal_step.toInt();
    
    uint32_t gDist = goal_dist.toInt();
    // Get current value
    uint32_t stp = get_fpy();
    uint32_t ach = gStep == 0 ? 0 : 100 * stp / gStep;
    uint32_t dist = stp * length.toInt() / 100;
     
    log_d("Refresh dashboard: %d FPY", stp);
   
    // Distance
    snprintf( buff, sizeof( buff ), "%s", "FPY - Shift" );
    lblDistance.text(buff).realign();
    // Achievement
    snprintf( buff, sizeof( buff ), "%d%%", gDist == 0 ? 0 : 100 * dist / gDist );
    lblDistachievement.text(buff).realign();
    arcDistance.end( gDist == 0 ? 0 : 360 * dist / gDist );


    snprintf( buff, sizeof( buff ), "%s", "FPY - Hour" );
    lblDistance2.text(buff).realign();
    
    snprintf( buff, sizeof( buff ), "%d%%", gDist == 0 ? 0 : 100 * dist / gDist );
    lblDistachievement2.text(buff).realign();
    arcDistance2.end( gDist == 0 ? 0 : 360 * dist / gDist );

}

void dashboard_activate_cb()
{
    refresh_main_page();
}

static void dashboard_reset_cb(lv_obj_t * obj, lv_event_t event)
{
    if(event == LV_EVENT_VALUE_CHANGED) {
        const char *answer = lv_msgbox_get_active_btn_text(obj);
        if ( strcmp( answer, YES) == 0 ) {
            /* Reset counter */
            bma_reset_stepcounter();
            /* Refresh display immediately for user feedback */
            refresh_main_page();
        }
    }
    /* Call the default callback to retrieve the default behavior */
    default_msgbox_cb(obj, event);
}

void build_settings()
{
    // Create full options list and attach items to variables
    config.addString("Line ID", 3, "None").setDigitsMode(true,"0123456789").assign(&length);            
    config.addString("Assembly", 8, "None").setDigitsMode(true,"0123456789").assign(&goal_step);       


    dashboardApp.useConfig(config, true); // true - auto create settings page widgets


}
