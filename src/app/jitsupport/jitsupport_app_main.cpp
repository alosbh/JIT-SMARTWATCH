...
void jitsupport_app_main_setup( uint32_t tile_num ) {
    ...
    lv_style_copy( &jitsupport_app_main_style, ws_get_mainbar_style());
  ...
  //-----Status Card------ 
    ...
    lv_obj_align(lbl_MQTT, status_card, LV_ALIGN_IN_TOP_LEFT, 5, 80);
     ...
     log_i("%s",NomeTopicoReceber);
      ...
      //***************************************************
          ...
          if(busca_ticket2(&atualiza)!=nullptr)
          {
              ...
              if(index>=0) 
              {
                printCard3(index,VIBRATION_SMOOTH);               
...
static void removefromArray(lv_obj_t *obj, lv_event_t event){
 ...
 if (event == LV_EVENT_CLICKED) {
    ...
    atual=get_number_tickets();
    printCard3(atual-1,VIBRATION_SMOOTH);
